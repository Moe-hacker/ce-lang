...
# Future:
Maybe we can have a `#[[ce_enforce(func)]]` to enforce you catch result for func in ce, and `:D` for ignoring the error, and `:o` for only log when error, and even `:~ { ... }` for a custom handler.       
